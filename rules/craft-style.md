...
- **Clean code**: Write for humans first — meaningful names, small focused functions, clear intent.

## Technical Debt

- **Log debt**: After each session, note tech debt found — `# TODO(debt):` inline or a ticket.
- **Debt types**: Reckless (corner-cut) vs. prudent (conscious trade-off with a plan). Log both. Treat differently.
...
- **Knowledge sharing**: Document decisions and insights; mentor others; contribute to community.
- **Continuous improvement**: Reflect on practices; experiment with new techniques; attend conferences.
...
- **Simplicity**: Prefer simple solutions; avoid premature optimization; keep functions short and shallow.
...
- **Tighten feedback loops**: Fast tests > slow tests. Local check > CI check. Immediate > delayed.
