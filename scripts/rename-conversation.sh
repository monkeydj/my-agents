...
# startup that cannot be avoided on subscription auth (`--bare` skips it but
# authenticates only via an API key). Running the hook async makes that cost
# invisible instead of shorter.
#
...
desc=$(generate_description) || desc=""
