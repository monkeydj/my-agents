...
`glab` next. (At the original fetch time the raw docs *index* in the `gitlab-org/cli` repository returned 404, and the older `editor_extensions/gitlab_cli/` page is a redirect stub stating the document "was moved to another location", pointing at `https://docs.gitlab.com/cli/`; the first two bullets below come from that page. On the 2026-09-07 verification pass the per-command raw pages under `docs/source/auth/` fetched normally and supply the third bullet.)
...
- Where `glab` stores a token entered via `glab auth login`, and whether it can use the macOS keychain — verified 2026-09-07 from the source-tree docs ([gitlab-org/cli, docs/source/auth/login.md](https://gitlab.com/gitlab-org/cli/-/raw/main/docs/source/auth/login.md)): by default the credential is stored in the operating system's keyring — macOS Keychain, Windows Credential Manager, or Linux Secret Service. When no keyring is available, or when `--insecure-storage` is passed ("Store token as plaintext in config file instead of OS keyring"), it is written as plaintext to `~/.config/glab-cli/config.yml`. In CI (when `GITLAB_CI` or `CI` is set) glab writes to the config file rather than the keyring. `GITLAB_TOKEN`, `GITLAB_ACCESS_TOKEN` or `OAUTH_TOKEN` take precedence over any stored credential, and a plaintext token in the config can be migrated into the keyring by running `glab auth login` again. The project README corroborates both locations — credentials live "in the configuration file (`$HOME/.config/glab-cli/config.yml`) or in your operating system's keyring" — and documents the lookup order: `GITLAB_TOKEN` first, then `GITLAB_ACCESS_TOKEN` ("Functionally equivalent to `GITLAB_TOKEN`"), then `OAUTH_TOKEN`, before stored credentials; `GLAB_CONFIG_DIR` relocates the config directory from its `~/.config/glab-cli/` default ([gitlab-org/cli, README.md](https://gitlab.com/gitlab-org/cli/-/raw/main/README.md)). Consequence for this design: **the default `glab auth login` on a Mac puts the token into the login keychain**, so it inherits every unattended-session problem described in 4.6–4.7 (locked keychain, access-allow dialog that cannot display under launchd or SSH). Do not rely on the stored login under launchd; export `GITLAB_TOKEN` from the secret store (4.7), which the docs confirm overrides the keyring entry. `glab auth status` exists ([docs/source/auth/status.md](https://gitlab.com/gitlab-org/cli/-/raw/main/docs/source/auth/status.md)) and checks the current-context instance, `--all` instances, or one `--hostname`; its exit code on a rejected token is not documented, so the health check should prefer a `git ls-remote` against one mirror, which exercises exactly the credential and scope the refresh uses.
...
Apple's own troubleshooting guidance confirms the two lock triggers relied on above, without documenting the unlock-at-login mechanism itself ([Apple Support, If your Mac keeps asking for the login keychain password](https://support.apple.com/guide/keychain-access/if-your-mac-keeps-asking-for-the-login-keychain-password-kyca1242/mac)): "Your keychain may be locked automatically if your computer has been inactive for a period of time or your user password and keychain password are out of sync." The page points at Keychain Access settings for the inactivity timer — the GUI equivalent of `set-keychain-settings -u -t`. The statement that the login keychain is unlocked automatically at GUI login is not on that page. On the 2026-09-07 verification pass an Apple-domain search located it in the macOS User Guide's keychain-password article ([Apple Support, About your keychain password](https://support.apple.com/guide/mac-help/about-your-keychain-password-mchlp1086/mac)): the login keychain's password normally matches the account login password, and when they match the keychain unlocks automatically at login; if the two differ, "it doesn't automatically unlock, and you're asked to enter the keychain's password." (Wording taken from search-engine excerpts of that page; a direct fetch of the URL returned only the guide's table-of-contents shell, not the article body, so the exact sentence could not be read in place — open the page in a browser to confirm before quoting it further.) This confirms the mechanism assumed above — the unlock is a *side effect of the GUI login with the account password* — and therefore also confirms why it is absent for a LaunchDaemon, an SSH login, and the pre-login window after reboot. The design above still does not depend on it: every recommended option in the table works whether or not the login keychain is unlocked.
...
(The rendered page at `sourcegraph.com/docs/admin/repo/update_frequency` returned HTTP 403 to this fetch, and the guessed path in the `sourcegraph-public-snapshot` repository returned 404; the facts below come from the same document at its `docs.sourcegraph.com` address and from Sourcegraph's engineering handbook, as surfaced by web search. Verification pass 2026-09-07: `raw.githubusercontent.com/sourcegraph/docs/main/docs/admin/repo/update_frequency.mdx` and `raw.githubusercontent.com/sourcegraph/sourcegraph-public-snapshot/main/doc/admin/repo/update_frequency.md` both returned 404, and `docs.sourcegraph.com/admin/repo/update_frequency` answered 302 to `https://sourcegraph.com/docs/admin/repo/update_frequency` — the same address that 403s. So the canonical URL is the `sourcegraph.com/docs/...` one; the quotes below were re-confirmed through search-engine excerpts of that page, not a direct fetch. See the Verification pass section at the end of this file.)
...
- Bounds: "Repositories will never be updated more frequently than 45 seconds, and no less frequently than every 8 hours." (Re-confirmed verbatim on 2026-09-07 through search-engine excerpts of the same page after both raw-GitHub paths 404ed — [Sourcegraph docs, Repository update frequency](https://docs.sourcegraph.com/admin/repo/update_frequency); the excerpts also state that after a repository's git data is updated "the global search index will automatically update a short while after (usually a few minutes)", and that `gitMaxCodehostRequestsPerSecond` caps "how many code host git operations can be run against a code host per second, per gitserver" — the rate-limit analogue of this document's `PULL_LIMIT`-style per-tick cap in 5.5.)
...
The usage page does not give the sync interval defaults or the `[mirror]` configuration keys. The rendered configuration cheat sheet truncated before its `[mirror]` section in the original fetch. On the 2026-09-07 verification pass the raw `app.example.ini` ([go-gitea/gitea, custom/conf/app.example.ini](https://raw.githubusercontent.com/go-gitea/gitea/main/custom/conf/app.example.ini)) returned the `[cron.update_mirrors]` block with its values:
...
So the walker runs every **10 minutes** and enqueues at most **50** pull and 50 push mirrors per pass; a mirror is only *eligible* when its own interval has elapsed, which is where the `[mirror]` keys (`DEFAULT_INTERVAL`, `MIN_INTERVAL`, `ENABLED`, `DISABLE_NEW_PULL`, `DISABLE_NEW_PUSH`) come in. The `[mirror]` block itself was **not** present in the content that fetch returned (the file is long and the extraction did not surface it), so its defaults were taken from the Go source that declares them ([go-gitea/gitea, modules/setting/mirror.go](https://raw.githubusercontent.com/go-gitea/gitea/main/modules/setting/mirror.go)): `Enabled: true`, `DisableNewPull: false`, `DisableNewPush: false`, `MinInterval: 10 * time.Minute`, `DefaultInterval: 8 * time.Hour`. `loadMirrorFrom` clamps `MinInterval` to at least 1 minute (with a warning) and, if `DefaultInterval` is set below `MinInterval`, resets it to `max(8h, MinInterval)`; `Enabled = false` disables both pull and push mirroring, and the legacy `[repository] DISABLE_MIRRORS` key maps onto `DisableNewPull` (deprecated since v1.19.0). Consequence: Gitea's defaults are the same shape as Sourcegraph's (5.1) — a **10-minute floor** and an **8-hour default ceiling** per mirror — and a 10-minute cron sweep means a mirror set to the minimum is refreshed roughly every 10–20 minutes, which brackets the 15–30 minute cadence this document recommends in Section 1.8. What the cron values establish is the architecture: a single cron task walks all mirrors on a schedule, with a per-run cap on how many pull and push mirrors it processes (`PULL_LIMIT`/`PUSH_LIMIT`), and `RUN_AT_START` performs a catch-up pass when the server boots.
...
Second pass over the four items this file had marked "I don't know" or unfetchable. Three were resolved from primary sources; one (Sourcegraph) is corroborated only through search-engine excerpts because every fetchable path to the page is blocked.

| # | Item | Outcome | Source |
|---|---|---|---|
| 3 | Where `glab` stores its token; macOS keychain support (Section 4.5) | **Resolved.** Default storage is the OS keyring — macOS Keychain, Windows Credential Manager, Linux Secret Service. Fallback (no keyring, or `--insecure-storage`, or `GITLAB_CI`/`CI` set) is plaintext in `~/.config/glab-cli/config.yml`. `GITLAB_TOKEN`, `GITLAB_ACCESS_TOKEN`, `OAUTH_TOKEN` override stored credentials, in that order. Re-running `glab auth login` migrates a plaintext token into the keyring. README corroborates the two locations and adds `GLAB_CONFIG_DIR` (default `~/.config/glab-cli/`). Section 4.5 bullet rewritten; consequence recorded that the default login lands in the login keychain and so inherits the 4.6–4.7 unattended-session problems. | [docs/source/auth/login.md](https://gitlab.com/gitlab-org/cli/-/raw/main/docs/source/auth/login.md); [README.md](https://gitlab.com/gitlab-org/cli/-/raw/main/README.md); [docs/source/auth/status.md](https://gitlab.com/gitlab-org/cli/-/raw/main/docs/source/auth/status.md) |
| 4 | Sourcegraph repository update frequency: min/max interval, backoff (Section 5.1) | **Not directly fetchable; corroborated.** Attempt 1 `raw.githubusercontent.com/sourcegraph/docs/main/docs/admin/repo/update_frequency.mdx` — 404. Attempt 2 `raw.githubusercontent.com/sourcegraph/sourcegraph-public-snapshot/main/doc/admin/repo/update_frequency.md` — 404. Attempt 3 `docs.sourcegraph.com/admin/repo/update_frequency` — 302 to `sourcegraph.com/docs/admin/repo/update_frequency`, the address that 403s. A web search returned excerpts of that page repeating verbatim "Repositories will never be updated more frequently than 45 seconds, and no less frequently than every 8 hours." and the `gitUpdateInterval` description ("a JSON array of repo name patterns and update intervals... If it matches no patterns a default backoff heuristic will be used"), plus `gitMaxCodehostRequestsPerSecond`. The 8 h → 4 h → 6 h worked example was not in the excerpts and stays as previously recorded. Inline note in 5.1 updated with the attempt log; the canonical URL is `https://sourcegraph.com/docs/admin/repo/update_frequency`. | [Sourcegraph docs, Repository update frequency](https://sourcegraph.com/docs/admin/repo/update_frequency) (via search excerpts) |
| 5 | Gitea/Forgejo `[mirror]` defaults and `[cron.update_mirrors]` values (Section 5.5) | **Resolved.** `app.example.ini` returned `[cron.update_mirrors]`: `SCHEDULE = @every 10m`, `ENABLED = true`, `RUN_AT_START = false`, `NOTICE_ON_SUCCESS = false`, `PULL_LIMIT=50`, `PUSH_LIMIT=50` (negative = no limit, 0 = effectively disabled). The `[mirror]` block did not surface from that file, so defaults were taken from the Go source: `Enabled: true`, `DisableNewPull: false`, `DisableNewPush: false`, `MinInterval: 10 * time.Minute`, `DefaultInterval: 8 * time.Hour`; `MinInterval` clamped to ≥ 1 min, `DefaultInterval` reset to `max(8h, MinInterval)` if set lower. Section 5.5 rewritten with both. | [custom/conf/app.example.ini](https://raw.githubusercontent.com/go-gitea/gitea/main/custom/conf/app.example.ini); [modules/setting/mirror.go](https://raw.githubusercontent.com/go-gitea/gitea/main/modules/setting/mirror.go) |
| 6 | Apple statement that the login keychain unlocks at login (Section 4.7) | **Located; wording from excerpts only.** An Apple-domain search surfaced the macOS User Guide article "About your keychain password", excerpted as: the login keychain password matches the account login password and unlocks automatically at login; if they differ, "it doesn't automatically unlock, and you're asked to enter the keychain's password." A direct fetch of the page returned only the guide's table-of-contents shell, so the sentence was not read in place. Section 4.7 now cites the article and marks the excerpt provenance. | [Apple Support, About your keychain password](https://support.apple.com/guide/mac-help/about-your-keychain-password-mchlp1086/mac) |

Still unverified after this pass: the exact Sourcegraph page text beyond the quoted bounds (blocked to automated fetch at every path tried), and the verbatim Apple sentence (page body not served to the fetcher). Both are cited to their canonical URLs for a human to open in a browser.

### Local verification on the target machine (2026-09-07)

Run on the user's Mac (`glab 1.112.0`, macOS Darwin 25.6). Observations, not documentation.

| Item | Observation |
|---|---|
| glab config location | No `~/.config/glab-cli/` exists. The live config is `~/Library/Application Support/glab-cli/config.yml` (mode 600). So the plaintext-fallback path cited from the docs in item 3 is the Linux/XDG path; on macOS glab uses the Application Support directory. |
| Token storage in use | `glab auth status --all` reports the working host as "Logged in ... **(keyring)**", and `config.yml` contains **zero** non-empty `token:` lines. The token is therefore in the login keychain on this machine, which confirms the Section 4.5-4.7 consequence: an unattended glab call with no GUI session may find the keychain locked. `security find-generic-password -s glab` found nothing, so glab's keychain service name is not simply `glab`; the exact item name was not determined. |
| `glab auth status` exit code | **1** whenever any configured host fails (one host here returns 401), and 1 with a bogus `GITLAB_TOKEN`. Pre-flight must pass `--hostname <host>` or a stale secondary host will fail every run. |
| Apple keychain-unlock statement | Not re-checked locally; item 6 stands as excerpt-sourced. |
